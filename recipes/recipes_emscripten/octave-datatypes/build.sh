#!/bin/bash
DATATYPES_DIR=$(pwd)/datatypes
SOURCE_DIR=$(pwd)/datatypes/src

export OCTAVE_VER="10.3.0"

# atm we install the *oct files in the same dir
# as the *.m files, without any arch-specific subdir
M_FILE_INSTALL_DIR=$PREFIX/share/octave/packages/datatypes-${PKG_VERSION}
...
cp "$DATATYPES_DIR"/{DESCRIPTION,COPYING,INDEX} "${M_FILE_INSTALL_DIR}/packinfo/"

# copy everything from $THIS_DIR/datatypes/into the M_FILE_INSTALL_DIR
cp -r "$DATATYPES_DIR"/inst/* "${M_FILE_INSTALL_DIR}/"

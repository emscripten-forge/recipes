...
def pin(dep: PkgName) -> str:
    """Version-pin a dependency on a sibling boost output (same 1.x minor
    line); external deps (zlib, bzip2) pass through unchanged."""
    return (f"${{{{ pin_subpackage('{dep}', upper_bound='x.x') }}}}"
            if dep.startswith("boost-") else dep)
...
def emit_outputs(blocks: list[dict[str, object]]) -> str:
    ...
    tail = "    tests: []\n"
    assert text.endswith(tail), "boost-cpp meta is the last block"
    text = text[: -len(tail)] + (
        "    # no test items: the meta package ships no files; its validation\n"
        "    # is dep-graph resolution succeeding + consumer regressions\n") + tail
    return text
...
def main(argv: list[str] | None = None) -> int:
    ...
    def compiled_block(m: ModuleKey) -> dict[str, object]:
        ...
        return {**pkg_head(f"boost-{m}", script, libs + claims[m] + [cfg_path(m)]),
                "requirements": {"run": [pin(d) for d in reqs] or None,
                                 "run_exports": [pin(f"boost-{m}")]},
    ...
    def meta_block(name: PkgName, roster: list[PkgName]) -> dict[str, object]:
        return {**pkg_head(name, 'echo "Collecting files from cache"'),
                "requirements": {"run": [pin(d) for d in roster],
                                 "run_exports": [pin(name)]},
                "tests": []}

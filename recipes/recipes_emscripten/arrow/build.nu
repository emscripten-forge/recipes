...
let ARROW_FEATURES = [
    ...
    $"-DBROTLI_ROOT=($env.PREFIX)"
    $"-DBrotliAlt_ROOT=($env.PREFIX)"
    $"-DBROTLI_COMMON_LIBRARY=($env.PREFIX)/lib/libbrotlicommon.a"
    $"-DBROTLI_ENC_LIBRARY=($env.PREFIX)/lib/libbrotlienc.a"
    $"-DBROTLI_DEC_LIBRARY=($env.PREFIX)/lib/libbrotlidec.a"
    $"-DBROTLI_INCLUDE_DIR=($env.PREFIX)/include"
    $"-DBZip2_ROOT=($env.PREFIX)"
    $"-DBZIP2_LIBRARIES=($env.PREFIX)/lib/libbz2.a"
    $"-DBZIP2_LIBRARY=($env.PREFIX)/lib/libbz2.a"
    $"-DBZIP2_INCLUDE_DIR=($env.PREFIX)/include"
    $"-Dlz4_ROOT=($env.PREFIX)"
    $"-Dre2_ROOT=($env.PREFIX)"
    $"-DZLIB_DIR=($env.PREFIX)"
    $"-DZLIB_ROOT=($env.PREFIX)"
    $"-DZLIB_LIBRARY=($env.PREFIX)/lib/libz.a"
    $"-DZLIB_INCLUDE_DIR=($env.PREFIX)/include"
    $"-Dabsl_ROOT=($env.PREFIX)"
    $"-Dnlohmann_json_ROOT=($env.PREFIX)"
    $"-DProtobuf_ROOT=($env.PREFIX)"
    $"-Dzstd_ROOT=($env.PREFIX)"
    $"-DZSTD_ROOT=($env.PREFIX)"
    $"-DZSTD_LIB=($env.PREFIX)/lib/libzstd.a"
    $"-DZSTD_INCLUDE_DIR=($env.PREFIX)/include"
    $"-Dxsimd_ROOT=($env.PREFIX)"
    $"-DRapidJSON_ROOT=($env.PREFIX)"
    $"-DThrift_DIR=($env.PREFIX)"
    $"-DThrift_ROOT=($env.PREFIX)"
    $"-DSnappy_ROOT=($env.PREFIX)"
    $"-DSnappy_DIR=($env.PREFIX)/lib/cmake/Snappy"

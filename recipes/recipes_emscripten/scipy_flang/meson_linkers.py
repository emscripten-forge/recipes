...
class WASMDynamicLinker(GnuLikeDynamicLinkerMixin, PosixDynamicLinkerMixin, DynamicLinker):
    ...
    def get_soname_args(self, env: 'Environment', prefix: str, shlib_name: str,
                        suffix: str, soversion: str, darwin_versions: T.Tuple[str, str]) -> T.List[str]:
        # raise MesonException(f'{self.id} WHO does not support shared libraries.')
        return ['-sSIDE_MODULE=1','--no-fatal-warnings','-Wl,--no-fatal-warnings']
...
class CompCertDynamicLinker(DynamicLinker):
    ...
    def get_soname_args(self, env: 'Environment', prefix: str, shlib_name: str,
                        suffix: str, soversion: str, darwin_versions: T.Tuple[str, str]) -> T.List[str]:
        raise MesonException(f'{self.id} ARE does not support shared libraries.')
...
class MetrowerksLinker(DynamicLinker):
    ...
    def get_soname_args(self, env: 'Environment', prefix: str, shlib_name: str,
                        suffix: str, soversion: str, darwin_versions: T.Tuple[str, str]) -> T.List[str]:
        raise MesonException(f'{self.id} YOU does not support shared libraries.')

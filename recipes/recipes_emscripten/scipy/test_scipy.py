...
def test_interpolate():
    ...
    y = np.sin(x)
    cs = CubicSpline(x, y)
    xs = np.arange(-0.3, 9.6, 1.0)
    assert_allclose(cs(xs), [-0.36576246, 0.65883730, 0.98512265, 0.42673135, -0.52968331, -0.99739603, -0.54927311, 0.40679813, 0.97823008, 0.68985652])
    assert_allclose(cs(xs, 1), [1.2900568, 0.71729263, -0.106572, -0.90408762, -0.85114626, -0.021004111, 0.83056275, 0.9153987, 0.16896908, -0.7710857])
    assert_allclose(cs(xs, 2), [-0.44721395, -0.69831441, -0.94941486, -0.40780254, 0.53088873, 0.9730695, 0.53212289, -0.43567411, -0.86394635, -1.0161632])
    assert_allclose(cs(xs, 3), [-0.25110045, -0.25110045, -0.25110045, 0.88134636, 0.96326767, 0.21885782, -0.72371994, -1.0724015, -0.15221686, -0.15221686])

...
if [[ ! -d "$BUILD_PREFIX/venv" ]]; then
  ...
  SYSCONFIG_FILE=$PREFIX/etc/conda/_sysconfigdata__emscripten_${WASM_FLAVOUR}-emscripten.py
  ...
  rm $BUILD_PREFIX/venv/lib/_sysconfigdata__emscripten_$WASM_FLAVOUR-emscripten.py
  cp $SYSCONFIG_FILE $BUILD_PREFIX/venv/lib
  cp $SYSCONFIG_FILE $BUILD_PREFIX/lib/python$PY_VER

fi

export _PYTHON_SYSCONFIGDATA_NAME="_sysconfigdata__emscripten_$WASM_FLAVOUR-emscripten"
...
# only use these when building on the mac, otherwise we mess up the linux build
  ...
  echo "Patching ctypes/__init__.py to avoid macOS specific code"
  CTYPES_INIT_FILE=$BUILD_PREFIX/lib/python$PY_VER/ctypes/__init__.py
  echo "CTYPES_INIT_FILE: $CTYPES_INIT_FILE"

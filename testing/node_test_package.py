...
def create_test_env(pkg_name, prefix):
    ...
    print("prefix",prefix)
    cmd = [f'$MAMBA_EXE create --yes --prefix {prefix} --platform=emscripten-32   python pytest_driver_node pytest {pkg_name}']
...
def test_package(recipe_dir, debug, debug_dir):
    ...
    if has_tests:
        ...
        with test_dir_context(pkg_name=pkg_name, debug=debug, debug_dir=debug_dir) as test_dir:
